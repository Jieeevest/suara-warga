...
export const AppProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const login = (username: string, password: string): boolean => {
    ...
    const validAdminUsername = import.meta.env.VITE_ADMIN_USERNAME || "admin";
    const validAdminPassword = import.meta.env.VITE_ADMIN_PASSWORD || "admin";
    ...
    // In a real app, password should be hashed/different. Here validation is NIK match.
    const resident = residents.find((r) => r.nik === username);
    if (resident && password === resident.nik) {
      // Password is NIK for now
      setCurrentUser({
        id: resident.id,
        name: resident.name,
        role: "resident",
